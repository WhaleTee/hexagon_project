...
bool rendering::system::initialize_render_pipeline::update() noexcept {
  mark_to_destroy();
...
bool rendering::system::release_render_pipeline::update() noexcept {
  using namespace component;
  ...
  const auto& pipeline_view = registry.view<render_pipeline_component, render_pipeline_release_request>();

  if (registry.valid(pipeline_view.front())) mark_to_destroy();

  for (auto&& [entity, pipeline]: pipeline_view.each()) {
