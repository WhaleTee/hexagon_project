...
bool rendering::system::create_window::update() noexcept {
  mark_to_destroy();
  ...
  const auto [window_flags, name, width, height] = window_setting_view.get<window_setting_component>(window_entity);
  ...
  const auto [shader_format, driver_name, debug] = window_setting_view.get<gpu_device_setting_component>(window_entity);
...
bool rendering::system::destroy_window::update() noexcept {
  using namespace component;
