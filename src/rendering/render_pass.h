...
namespace rendering::system {
  struct begin_render_pass final : ecs::system::base_system {
    ...
    bool update() noexcept override {
      ...
      const auto render_cmd_buffer_view = registry.view<gpu_command_buffer_component, entt::tag<component_tag::render>, gpu_color_target_setting_component>();
      ...
      auto [command_buffer, color_target_setting] = render_cmd_buffer_view.get<gpu_command_buffer_component, gpu_color_target_setting_component>(render_cmd_buffer_entity);
      const auto window_view = registry.view<window_component, window_setting_component>();
      ...
      auto [window, window_setting] = window_view.get<window_component, window_setting_component>(window_entity);

      if (!SDL_AcquireGPUSwapchainTexture(command_buffer.value, window.value, &color_target_setting.swapchainTexture, nullptr, nullptr)) {
      ...
      if (color_target_setting.swapchainTexture != nullptr) {
        ...
        registry.emplace<gpu_render_pass_component>(render_cmd_buffer_entity, SDL_BeginGPURenderPass(command_buffer.value, &color_target_info, 1, nullptr));
      }
